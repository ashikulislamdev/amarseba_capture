...
class FaceVerifyScreen extends StatefulWidget {
  ...
  const FaceVerifyScreen(
      {super.key, required this.code, required this.username});
  ...
  Future<void> _sendDataToServer() async {
    if (_capturedImage == null) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
            content: Text(
          'কোনো ছবি ধারণ করা হয়নি',
          style: TextStyle(fontFamily: 'NikoshBAN'),
        )),
      );
      return;
    }
    ...
    request.files.add(await http.MultipartFile.fromPath(
        'profile_photo_path', _capturedImage!.path));
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
              content: Text('প্রোফাইল আপডেট করতে ব্যর্থ হয়েছে',
                  style: TextStyle(fontFamily: 'NikoshBAN'))),
        );
      }
    } catch (e) {
